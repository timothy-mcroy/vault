...
- `url` `(string: ldap://127.0.0.1)` – The LDAP server to connect to. Examples:
  ...
  specified with commas, e.g. `ldaps://ldap.myorg.com,ldaps://ldap2.myorg.com`;
  these will be tried in-order. For production configurations, prefer ldaps://
  as [ldap:// sends credentials to the LDAP server via cleartext.](https://docs.microsoft.com/en-us/archive/blogs/pki/implementing-ldaps-ldap-over-ssl)
